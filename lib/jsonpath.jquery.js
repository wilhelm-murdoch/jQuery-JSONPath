(function($){
  $.jsonPath = function(options) {
    var options = $.extend({
      ...
      resultType: 'VALUE',
      keepHistory: true,
      onFound: function(item, object) {},
      onNormalize: function(expr) {},
      onAsPath: function(path) {},
      onTrace: function(expr, val, path) {},
      onWalk: function(loc, expr, val, path, f) {},
      onSlice: function(loc, expr, val, path) {},
      onEval: function(x, _v, _vname) {}
    }, options);
    ...
    var P = {
      ...
      store: function(p, v) {
        ...
        if (p) P.result[P.result.length] = options.resultType == 'PATH' ? P.asPath(p) : v;
        return !!p;
      },
      ...
      walk: function(loc, expr, val, path, f) {
        options.onWalk(loc, expr, val, path, f)
      ...
      slice: function(loc, expr, val, path) {
        options.onSlice(loc, expr, val, path)
      ...
      eval: function(x, _v, _vname) {
        options.onEval(loc, expr, val, path)
    ...
    this.find = function(expr) {
      ...
      if (expr) {
        P.trace(P.normalize(expr).replace(/^\$;?/,''), options.data, '$');  // issue 6 resolved
